...
<main>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="search-header mb-4">
                        ...
                        <p class="text-muted"><?= !empty($lastPosts) ? count($lastPosts) : 0 ?> articles found</p>
                    </div>
                </div>
            </div>
                    ...
                    <article class="hero-main-post">
                        ...
                        <div class="hero-overlay">
                            <span class="hero-category">
                                <i class="<?= getCategoryIcon($lastPosts[0]['category_name'] ?? '') ?>"></i> <?= $lastPosts[0]['category_name'] ?? '' ?>
                            </span>
                            <h1 class="hero-title">
    ...
    <section class="py-5">
        <div class="container">
            ...
            <?php 
            // �?m b?o c� d? d? li?u cho Most Viewed
            $allMostViewed = [];
            
            // K?t h?p c? popularPosts v� lastPosts d? c� d? d? li?u
            ...
            // Lo?i b? duplicate v� l?y 9 b�i d?u ti�n
            $uniquePosts = [];
            $seenIds = [];
            ...
            // L?y 3 b�i view cao nh?t cho top section
            $topViewed = array_slice($uniquePosts, 0, 3);
            // L?y c�c b�i c�n l?i cho bottom section (2 c?t)
            ...
            <div class="row g-3">
                <?php 
                // N?u kh�ng c� d? bottomViewed, s? d?ng th�m t? lastPosts
    ...
    <section class="py-5" style="background-color: #f8f9fa;">
        <div class="container">
            <?php 
            // L?y t?t c? categories c� b�i vi?t
            $featuredCategories = [];
            
            // N?u c� d? li?u categories t? database
            if (!empty($categories)) {
                foreach($categories as $category) {
                    $featuredCategories[] = $category['name'];
                }
            } else {
                // Fallback: L?y categories t? b�i vi?t hi?n c�
                $uniqueCategories = array_unique(array_column($lastPosts, 'category'));
                $featuredCategories = array_slice($uniqueCategories, 0, 6); // L?y t?i da 6 categories
            }
            
            // N?u v?n kh�ng c�, s? d?ng default
            ...
            foreach($featuredCategories as $categoryName) {
                // L?c b�i vi?t theo category
                $categoryPosts = array_filter($lastPosts, function($post) use ($categoryName) {
                    return stripos($post['category_name'] ?? '', $categoryName) !== false;
                });
                
                // N?u kh�ng c� b�i vi?t c?a category n�y, b? qua
                if (empty($categoryPosts)) continue;
                
                // Hi?n th? T?T C? b�i vi?t c� trong category (kh�ng gi?i h?n)
                // $categoryPosts = array_slice($categoryPosts, 0, 8); // B? gi?i h?n
            ?>
                    ...
                    <article class="news-card">
                        <div class="news-card-image">
                            ...
                            <span class="news-card-category" style="background-color: <?= getCategoryColor($categoryName) ?>;">
                                <i class="<?= getCategoryIcon($categoryName) ?>"></i> <?= $post['category_name'] ?? '' ?>
                            </span>
                        </div>
                        <div class="news-card-content">
                            <h3 class="news-card-title">
            ...
            <?php 
            // N?u kh�ng c� b�i vi?t n�o t? featured categories, hi?n th? fallback
            $hasAnyPosts = false;
            foreach($featuredCategories as $cat) {
                $filtered = array_filter($lastPosts, function($post) use ($cat) {
                    return stripos($post['category_name'] ?? '', $cat) !== false;
                });
                if (!empty($filtered)) {
                    $hasAnyPosts = true;
                    break;
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-4 col-md-6">
                    <article class="news-card">
                        <div class="news-card-image">
                            <img src="<?= asset($post['image']) ?>" alt="<?= $post['title'] ?>">
                            <span class="news-card-category" style="background-color: <?= getCategoryColor($post['category_name'] ?? '') ?>;">
                                <i class="<?= getCategoryIcon($post['category_name'] ?? '') ?>"></i> <?= $post['category_name'] ?? '' ?>
                            </span>
                        </div>
                        <div class="news-card-content">
                            <h3 class="news-card-title">
