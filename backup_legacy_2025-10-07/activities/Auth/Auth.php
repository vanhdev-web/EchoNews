...
class Auth
{
    ...
    public function sendMail($emailAddress, $subject, $body)
    {
        ...
        try {
            ...
            file_put_contents('mail_errors.log', date('Y-m-d H:i:s') . " - Mail Error: " . $mail->ErrorInfo . " | Exception: " . $e->getMessage() . "\n", FILE_APPEND);
            return false;
        }
    }
        ...
        if (empty($request['email']) || empty($request['username']) || empty($request['password'])) {
            ...
            if ($user != null) {
                ...
                if ($result) {
                    ...
                    flash('register_success', 'Registration successful! Please check your email to activate your account.');
                    $this->redirect('login');
                } else {
                    // More specific error message
                    $errorDetails = '';
                    if (file_exists('mail_errors.log')) {
                        $errorLog = file_get_contents('mail_errors.log');
                        $lastError = trim(substr($errorLog, strrpos($errorLog, "\n", -2)));
                        if (strpos($lastError, 'Could not authenticate') !== false) {
                            $errorDetails = ' Error: Invalid Mailtrap credentials. Please check your username and password.';
                        } elseif (strpos($lastError, 'Connection refused') !== false) {
                            $errorDetails = ' Error: Cannot connect to mail server. Please check your network connection.';
                        } elseif (strpos($lastError, 'timed out') !== false) {
                            $errorDetails = ' Error: Connection timeout. Please try a different port (2525, 587, or 25).';
                        }
                    }
                    flash('register_error', 'The activation email could not be sent. Please check your email configuration.' . $errorDetails);
                    $this->redirectBack();
                }
            }
        }
